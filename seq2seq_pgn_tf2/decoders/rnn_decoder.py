...
class BahdanauAttentionCoverage(tf.keras.layers.Layer):
    ...
    def call(self, dec_hidden, enc_output, enc_padding_mask, use_coverage=False, prev_coverage=None):
        ...
        def masked_attention(score):
            """1.做正常的softmax概率的归一化，2.使用mask填充不需要计算的位置，提升attension的计算效率
            ...
            attn_dist *= mask #把attn_dist mask掉，乘0，得0
            masked_sums = tf.reduce_sum(attn_dist, axis=1) # 沿一轴 所有元素相加，并且降维成 (16,)
        ...
        if use_coverage and prev_coverage is not None:  # non-first step of coverage
            ...
            # 加入coverge 来改造seq2seq中的score，即score中又加了前t时刻注意力权重之和 与其W矩阵的乘积，
            e = self.V(tf.nn.tanh(self.W1(enc_output) + self.W2(hidden_with_time_axis) + self.Wc(prev_coverage)))
            # Calculate attention distribution
            attn_dist = masked_attention(e)
            # Update coverage vector, = t时刻的coverge + 前t时刻所有coverge
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, hidden, enc_output, context_vector):
        ...
        x = tf.concat([tf.expand_dims(context_vector, 1), x], axis=-1) # shape=(batch_size, 1, ec_units*2)
...
class Pointer(tf.keras.layers.Layer):
    ...
    def call(self, context_vector, state, dec_inp):
        # Pen系数 = sigmoid(w_c矩阵的转置*context_vector + w_s矩阵的转置*decoder隐藏层向量 + w_x矩阵的转置*decoder的输入)
